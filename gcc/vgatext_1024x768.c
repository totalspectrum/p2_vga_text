// automatically generated by spin2cpp v3.9.21 on Mon Mar  4 19:14:04 2019
// command line: spin2cpp --p2 --ccode vgatext_1024x768.spin2 

//
// clock frequency settings
// for 1024x768 we use a 65 MHz pixel clock
// and clock the P2 to 200 MHz
#include <propeller.h>
#include "vgatext_1024x768.h"

#ifdef __GNUC__
#define INLINE__ static inline
#else
#define INLINE__ static
#define waitcnt(n) _waitcnt(n)
#define coginit(id, code, par) _coginit((unsigned)(par)>>2, (unsigned)(code)>>2, id)
#define cognew(code, par) coginit(0x8, (code), (par))
#define cogstop(i) _cogstop(i)
#endif

INLINE__ int32_t Shr__(uint32_t a, uint32_t b) { return (a>>b); }
static  void vgatext_1024x768_init_terminal(vgatext_1024x768 *self);
static  void vgatext_1024x768_reset_graphics_state(vgatext_1024x768 *self);
static  void vgatext_1024x768_scrollup(vgatext_1024x768 *self);
static  void vgatext_1024x768_checkposition(vgatext_1024x768 *self);
static  void vgatext_1024x768_handle_esc(vgatext_1024x768 *self, int32_t c);
static  int32_t vgatext_1024x768_defaultval(int32_t arg, int32_t v);
static  void vgatext_1024x768_handle_csi(vgatext_1024x768 *self, int32_t c);
static  void vgatext_1024x768_clear(vgatext_1024x768 *self, int32_t a, int32_t b);
static  void vgatext_1024x768_clear_in_display(vgatext_1024x768 *self, int32_t n);
static  int32_t vgatext_1024x768_fetcharg(vgatext_1024x768 *self, int32_t i);
static  void vgatext_1024x768_set_graphic_rendition(vgatext_1024x768 *self, int32_t i);
static  int32_t vgatext_1024x768_ansicolor(vgatext_1024x768 *self, int32_t n);
static  int32_t vgatext_1024x768_sixcolor(int32_t x);
static  Tuple2__ vgatext_1024x768_getargcolor(vgatext_1024x768 *self, int32_t i);
static  int32_t vgatext_1024x768_calcscale(int32_t a, int32_t b);

static char dat[] = {
  , 
};
int32_t vgatext_1024x768_start(vgatext_1024x768 *self, int32_t pinbase)
{
  int32_t 	i, pclkscale, pclk, sysclk, x;
  // calculate clock frequency
  // pixel clock
  pclk = VGATEXT_1024X768_PIXEL_CLOCK_FREQ;
  // system clock
  sysclk = _clkfreq;
  // calculate scale := $8000_0000 * pclk / sysclk
  // this is equal to pclk / (sysclk / $8000_000)
  pclkscale = vgatext_1024x768_calcscale(pclk, sysclk);
  i = 0;
  self->params[(i++)] = pinbase;
  // screen buffer
  self->params[(i++)] = (int32_t)(self->screen_buffer);
  // screen columns
  self->params[(i++)] = VGATEXT_1024X768_COLS;
  // screen rows
  self->params[(i++)] = VGATEXT_1024X768_ROWS;
  // font data
  self->params[(i++)] = (int32_t)(((char *)&dat[0]));
  // font width
  self->params[(i++)] = 8;
  // font height
  self->params[(i++)] = VGATEXT_1024X768_FONT_HEIGHT;
  // fset           ' pixel clock scaling value
  self->params[(i++)] = pclkscale;
  // horizontal front porch
  self->params[(i++)] = 24;
  // hsync pulse
  self->params[(i++)] = 136;
  // horizontal back porch
  self->params[(i++)] = 160;
  // vertical front porch
  self->params[(i++)] = 3;
  // vertical sync lines
  self->params[(i++)] = 6;
  // vertical back porch
  self->params[(i++)] = 29;
  // vertical/horizontal polarity
  self->params[(i++)] = 0;
  x = vga_tile_driver_start(&self->vga, (int32_t)(self->params));
  vgatext_1024x768_init_terminal(self);
  return x;
}

void vgatext_1024x768_stop(vgatext_1024x768 *self)
{
  vga_tile_driver_stop(&self->vga);
}

static void vgatext_1024x768_init_terminal(vgatext_1024x768 *self)
{
  vgatext_1024x768_reset_graphics_state(self);
  vgatext_1024x768_cls(self);
  self->curx = self->cury = 0;
  self->state = VGATEXT_1024X768_STATE_NORMAL;
  self->screenptr = (int32_t)(self->screen_buffer);
}

static void vgatext_1024x768_reset_graphics_state(vgatext_1024x768 *self)
{
  self->bgcolor = VGATEXT_1024X768_DEFAULT_BG_COLOR;
  self->fgcolor = VGATEXT_1024X768_DEFAULT_FG_COLOR;
  self->char_effects = 0;
}

static void vgatext_1024x768_scrollup(vgatext_1024x768 *self)
{
  memcpy( (void *)self->screenptr, (void *)(void *)((self->screenptr + (8 * VGATEXT_1024X768_COLS))), 1*(((8 * (VGATEXT_1024X768_ROWS - 1)) * VGATEXT_1024X768_COLS)));
  memset( (void *)(void *)((self->screenptr + ((8 * (VGATEXT_1024X768_ROWS - 1)) * VGATEXT_1024X768_COLS))), 0, 1*((8 * VGATEXT_1024X768_COLS)));
}

static void vgatext_1024x768_checkposition(vgatext_1024x768 *self)
{
  if (self->curx >= VGATEXT_1024X768_COLS) {
    self->curx = 0;
    (self->cury++);
  } else {
    if (self->curx < 0) {
      self->curx = 0;
    }
  }
  if (self->cury >= VGATEXT_1024X768_ROWS) {
    vgatext_1024x768_scrollup(self);
    self->cury = VGATEXT_1024X768_ROWS - 1;
  } else {
    if (self->cury < 0) {
      self->cury = 0;
    }
  }
}

void vgatext_1024x768_tx(vgatext_1024x768 *self, int32_t c)
{
  int32_t 	fg, bg;
  if (self->state == VGATEXT_1024X768_STATE_NORMAL) {
    if (c == 8) {
      // backspace
      (--self->curx);
    } else {
      if (c == 10) {
        // line feed
        (self->cury++);
      } else {
        if (c == 13) {
          // carriage return
          self->curx = 0;
        } else {
          if (c == 20) {
            // tab
            self->curx = (self->curx + 8) & ((7 << 24) >> 24);
          } else {
            if (c == 27) {
              self->state = VGATEXT_1024X768_STATE_ESCAPE;
            } else {
              fg = self->fgcolor;
              bg = self->bgcolor;
              if (self->char_effects & VGATEXT_1024X768_CHAR_INVERSE) {
                // swap
                { Tuple2__ tmp__ = ((Tuple2__){bg, fg}); fg = tmp__.v0; bg = tmp__.v1;  };
              }
              vgatext_1024x768_checkposition(self);
              vgatext_1024x768_glyphat(self, self->curx, self->cury, c, fg, bg);
              (self->curx++);
            }
          }
        }
      }
    }
  } else {
    if (self->state == VGATEXT_1024X768_STATE_ESCAPE) {
      vgatext_1024x768_handle_esc(self, c);
    } else {
      if (self->state == VGATEXT_1024X768_STATE_CSI) {
        vgatext_1024x768_handle_csi(self, c);
      }
    }
  }
}

static void vgatext_1024x768_handle_esc(vgatext_1024x768 *self, int32_t c)
{
  int32_t 	i;
  if (c == '[') {
    for(i = 0; i < VGATEXT_1024X768_MAX_ESC_ARGS; i++) {
      self->args[i] = 0;
    }
    self->argidx = 0;
    self->state = VGATEXT_1024X768_STATE_CSI;
  } else {
    if (c == 'c') {
      vgatext_1024x768_init_terminal(self);
    } else {
      self->state = VGATEXT_1024X768_STATE_NORMAL;
    }
  }
}

static int32_t vgatext_1024x768_defaultval(int32_t arg, int32_t v)
{
  return ((arg) ? arg : v);
}

// handle ANSI control sequences
static void vgatext_1024x768_handle_csi(vgatext_1024x768 *self, int32_t c)
{
  /* 
      glyphat(40, 10, c, $ffff0000, $00404000)
      pausems(1000)
   */
  if ((c >= '0') && (c <= '9')) {
    self->args[self->argidx] = (10 * self->args[self->argidx]) + (c - '0');
    return;
  }
  (self->argidx++);
  if (self->argidx == VGATEXT_1024X768_MAX_ESC_ARGS) {
    (--self->argidx);
  }
  if (c == ';') {
    return;
  }
  if (c == 'A') {
    self->cury = self->cury - vgatext_1024x768_defaultval(self->args[0], 1);
  } else {
    if (c == 'B') {
      self->cury = self->cury + vgatext_1024x768_defaultval(self->args[0], 1);
    } else {
      if (c == 'C') {
        self->curx = self->curx + vgatext_1024x768_defaultval(self->args[0], 1);
      } else {
        if (c == 'D') {
          self->curx = self->curx - vgatext_1024x768_defaultval(self->args[0], 1);
        } else {
          if (c == 'E') {
            self->cury = self->cury + vgatext_1024x768_defaultval(self->args[0], 1);
            self->curx = 0;
          } else {
            if (c == 'F') {
              self->cury = self->cury - vgatext_1024x768_defaultval(self->args[0], 1);
              self->curx = 0;
            } else {
              if (c == 'G') {
                self->curx = vgatext_1024x768_defaultval(self->args[0], 1) - 1;
              } else {
                if (c == 'H') {
                  self->cury = vgatext_1024x768_defaultval(self->args[0], 1) - 1;
                  self->curx = vgatext_1024x768_defaultval(self->args[1], 1) - 1;
                } else {
                  if (c == 'J') {
                    vgatext_1024x768_clear_in_display(self, vgatext_1024x768_defaultval(self->args[0], 0));
                  } else {
                    if (c == 'm') {
                      vgatext_1024x768_set_graphic_rendition(self, 0);
                    } else {
                      if (c == 's') {
                        self->savex = self->curx;
                        self->savey = self->cury;
                      } else {
                        if (c == 'u') {
                          self->curx = self->savex;
                          self->cury = self->savey;
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
  }
  self->state = VGATEXT_1024X768_STATE_NORMAL;
}

void vgatext_1024x768_glyphat(vgatext_1024x768 *self, int32_t x, int32_t y, int32_t ch, int32_t fgcol, int32_t bgcol)
{
  int32_t 	bufptr;
  bufptr = (int32_t)(self->screen_buffer);
  bufptr = bufptr + (((y * VGATEXT_1024X768_COLS) + x) * 8);
  fgcol |= (ch & 0xff);
  bgcol &= 0xffffff00;
  ((int32_t *)bufptr)[0] = fgcol;
  bufptr = bufptr + 4;
  ((int32_t *)bufptr)[0] = bgcol;
}

void vgatext_1024x768_cls(vgatext_1024x768 *self)
{
  memset( (void *)(self->screen_buffer), 0, sizeof(int32_t)*((VGATEXT_1024X768_COLS * VGATEXT_1024X768_ROWS) * 2));
}

// clear from position a to position b (inclusive)
static void vgatext_1024x768_clear(vgatext_1024x768 *self, int32_t a, int32_t b)
{
  int32_t 	ptr, cnt;
  ptr = ((int32_t)(self->screen_buffer)) + a;
  cnt = b - a;
  while (cnt >= 0) {
    ((int32_t *)ptr)[0] = self->fgcolor;
    ptr = ptr + 4;
    ((int32_t *)ptr)[0] = self->bgcolor;
    ptr = ptr + 4;
    (--cnt);
  }
}

// clear from cursor to end of display (if n==0) or from start of screen to cursor (n == 1)
// n==2 clears whole display and resets cursor to top
static void vgatext_1024x768_clear_in_display(vgatext_1024x768 *self, int32_t n)
{
  int32_t 	startpix, endpix;
  if (n == 2) {
    self->curx = self->cury = 0;
    n = 0;
  }
  if (n == 1) {
    startpix = 0;
    endpix = (VGATEXT_1024X768_COLS * self->cury) + self->curx;
  } else {
    startpix = (VGATEXT_1024X768_COLS * self->cury) + self->curx;
    endpix = (VGATEXT_1024X768_COLS * VGATEXT_1024X768_ROWS) - 1;
  }
  vgatext_1024x768_clear(self, startpix, endpix);
}

// get an argument that's already been set up, return -1 if not valid
static int32_t vgatext_1024x768_fetcharg(vgatext_1024x768 *self, int32_t i)
{
  if (i >= self->argidx) {
    return (-1);
  }
  return self->args[i];
}

static void vgatext_1024x768_set_graphic_rendition(vgatext_1024x768 *self, int32_t i)
{
  int32_t 	code, a, b;
  if (i > self->argidx) {
    return;
  }
  code = vgatext_1024x768_fetcharg(self, i++);
  /*   
    a := hexdig(code & $F)
    b := hexdig((code >> 4) & $F)
    glyphat(41, 10, b, $ffffff00, $00202000)
    glyphat(42, 10, a, $ffffff00, $00202000)
    pausems(1000)
   */
  if (code == 0) {
    vgatext_1024x768_reset_graphics_state(self);
  } else {
    if (code == 1) {
      self->char_effects |= VGATEXT_1024X768_CHAR_BOLD;
    } else {
      if (code == 7) {
        self->char_effects |= VGATEXT_1024X768_CHAR_INVERSE;
      } else {
        if (code == 21) {
          self->char_effects &= (~VGATEXT_1024X768_CHAR_BOLD);
        } else {
          if (code == 22) {
            self->char_effects &= (~VGATEXT_1024X768_CHAR_BOLD);
          } else {
            if (code == 27) {
              self->char_effects &= (~VGATEXT_1024X768_CHAR_INVERSE);
            } else {
              if ((code >= 30) && (code <= 37)) {
                self->fgcolor = vgatext_1024x768_ansicolor(self, code - 30);
              } else {
                if (code == 38) {
                  { Tuple2__ tmp__ = vgatext_1024x768_getargcolor(self, i); self->fgcolor = tmp__.v0; i = tmp__.v1;  };
                } else {
                  if (code == 39) {
                    self->fgcolor = VGATEXT_1024X768_DEFAULT_FG_COLOR;
                  } else {
                    if ((code >= 40) && (code <= 47)) {
                      self->bgcolor = vgatext_1024x768_ansicolor(self, code - 40);
                    } else {
                      if (code == 48) {
                        { Tuple2__ tmp__ = vgatext_1024x768_getargcolor(self, i); self->bgcolor = tmp__.v0; i = tmp__.v1;  };
                      } else {
                        if (code == 49) {
                          self->bgcolor = VGATEXT_1024X768_DEFAULT_BG_COLOR;
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
  }
  vgatext_1024x768_set_graphic_rendition(self, i);
}

// look up an ANSI color from the color table
static int32_t vgatext_1024x768_ansicolor(vgatext_1024x768 *self, int32_t n)
{
  int32_t 	ptr;
  if (self->char_effects & VGATEXT_1024X768_CHAR_BOLD) {
    n = n + 8;
  }
  ptr = (int32_t)(((int32_t *)&dat[4140]));
  return ((int32_t *)(ptr + (4 * n)))[0];
}

// calculate an RGB color from a 6,6,6 encoded 8 bit color
static int32_t vgatext_1024x768_sixcolor(int32_t x)
{
  int32_t 	r, g, b;
  b = x % 6;
  x = x / 6;
  g = x % 6;
  r = x / 6;
  r = (255 * r) / 6;
  g = (255 * g) / 6;
  b = (255 * b) / 6;
  return (((r << 24) | (g << 16)) | b);
}

// get a color out of arguments
static Tuple2__ vgatext_1024x768_getargcolor(vgatext_1024x768 *self, int32_t i)
{
  int32_t 	r, g, b, code;
  code = vgatext_1024x768_fetcharg(self, i++);
  if (code == 0) {
    return ((Tuple2__){0, i});
  }
  if (code == 2) {
    r = vgatext_1024x768_fetcharg(self, i++) & 0xff;
    g = vgatext_1024x768_fetcharg(self, i++) & 0xff;
    b = vgatext_1024x768_fetcharg(self, i++) & 0xff;
    return ((Tuple2__){((r << 24) | (g << 16)) | (b << 8), i});
  }
  if (code == 5) {
    r = vgatext_1024x768_fetcharg(self, i++) & 0xff;
    if (r <= 15) {
      r = ((int32_t *)&dat[4140])[r];
    } else {
      if (r >= 232) {
        r = 10 * (r - 232);
        (r + (-8));
        return ((Tuple2__){((r << 24) | (r << 16)) | (r << 8), i});
      } else {
        r = vgatext_1024x768_sixcolor(r);
        return ((Tuple2__){r, i});
      }
    }
  }
  // punt for now
  return ((Tuple2__){0, i});
}

static int32_t vgatext_1024x768_calcscale(int32_t a, int32_t b)
{
  int32_t 	shiftcnt;
  shiftcnt = VGATEXT_1024X768_PIXSHIFT;
  // remove factors of 5 (will be pretty common)
  while ((0 == (a % 5)) && (0 == (b % 5))) {
    a = a / 5;
    b = b / 5;
  }
  // scale a up as much as we can
  while (((a & 0x40000000) == 0) && (shiftcnt > 0)) {
    a = a << 1;
    (shiftcnt--);
  }
  while (((b & 0x1) == 0) && (shiftcnt > 0)) {
    b = Shr__(b, 1);
    (shiftcnt--);
  }
  return ((a / b) << shiftcnt);
}

void vgatext_1024x768_str(vgatext_1024x768 *self, const char *s)
{
  int32_t 	c;
  while ((c = (s++)[0]) != 0) {
    vgatext_1024x768_tx(self, c);
  }
}

void vgatext_1024x768_num(vgatext_1024x768 *self, int32_t val, int32_t base, int32_t signflag, int32_t digitsneeded)
{
  int32_t 	i, digit, r1, q1;
  // if signflag is nonzero, it indicates we should treat
  // val as signed; if it is > 1, it is a character we should
  // print for positive numbers (typically "+")
  if (signflag) {
    if (val < 0) {
      signflag = '-';
      val = -val;
    }
  }
  // make sure we will not overflow our buffer
  if (digitsneeded > 32) {
    digitsneeded = 32;
  }
  // accumulate the digits
  i = 0;
  do {
    if (val < 0) {
      // synthesize unsigned division from signed
      // basically shift val right by 2 to make it positive
      // then adjust the result afterwards by the bit we
      // shifted out
      // capture low bit
      r1 = val & 0x1;
      // divide val by 2
      q1 = Shr__(val, 1);
      digit = r1 + (2 * (q1 % base));
      val = 2 * (q1 / base);
      if (digit >= base) {
        (val++);
        digit = digit - base;
      }
    } else {
      digit = val % base;
      val = val / base;
    }
    if ((digit >= 0) && (digit <= 9)) {
      digit = digit + '0';
    } else {
      digit = (digit - 10) + 'A';
    }
    self->buf[(i++)] = digit;
    (--digitsneeded);
  } while (((val != 0) || (digitsneeded > 0)) && (i < 32));
  if (signflag > 1) {
    vgatext_1024x768_tx(self, signflag);
  }
  // now print the digits in reverse order
  while (i > 0) {
    vgatext_1024x768_tx(self, self->buf[(--i)]);
  }
}

// print a signed decimal number
void vgatext_1024x768_dec(vgatext_1024x768 *self, int32_t val)
{
  vgatext_1024x768_num(self, val, 10, 1, 0);
}

// print an unsigned decimal number with the specified
// number of digits; 0 means just use as many as we need
void vgatext_1024x768_decuns(vgatext_1024x768 *self, int32_t val, int32_t digits)
{
  vgatext_1024x768_num(self, val, 10, 0, digits);
}

// print a hex number with the specified number
// of digits; 0 means just use as many as we need
void vgatext_1024x768_hex(vgatext_1024x768 *self, int32_t val, int32_t digits)
{
  int32_t 	mask;
  vgatext_1024x768_num(self, val, 16, 0, digits);
}

// print a newline
void vgatext_1024x768_nl(vgatext_1024x768 *self)
{
  vgatext_1024x768_tx(self, 13);
  vgatext_1024x768_tx(self, 10);
}

